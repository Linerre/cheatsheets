...
\item $S$ is non-terminal, need to add new items to $I_0$
  ...
  %TODO: move this to another standalone sheet
%   \begin{enumerate}
%   \item if prev item prod $A\to \lrd S\beta\$$, $\beta$ is a \mo{terminal}
%     \[
%       \textsf{LA} = \mfn{first}(\beta\$) = \mfn{first}(\beta)
%       \underset{\beta \overset{*}{\Rightarrow} \varepsilon}{\overset{\text{iff}}{\bigcup}}\mfn{first}(\$)
%     \]
%   \item if prev item pord $A\to \lrd SX\$$, $X$ is \mb{non-terminal}
%     \[
% \textsf{LA} = \mfn{first}(X\$) = \mfn{first}(X) \underset{X \overset{*}{\Rightarrow} \varepsilon}{\overset{\text{iff}}{\bigcup}}\mfn{first}(\$)
%     \]
%   \item if prev item pord $A\to \lrd SX\beta\$$, $X$ \mb{non-term}, $\beta$ \mb{term}
%     \[
% \textsf{LA} = \mfn{first}(X\beta) = \mfn{first}(X) \underset{X \overset{*}{\Rightarrow} \varepsilon}{\overset{\text{iff}}{\bigcup}}\mfn{first}(\beta)
%     \]
%   \item if prev item prod $A\to \lrd S\$$, LA = $\mfn{first}(\$)$ (our case)
%   \end{enumerate}
