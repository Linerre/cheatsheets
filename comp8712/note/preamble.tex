% -- Layout ----
\usepackage[top=0.6cm, bottom=0.6cm, left=0.5cm, right=0.5cm, landscape]{geometry}

% -- Titles ----
\usepackage[
  tiny,                     % text size title
  compact                   % reduce vertical space before/after title
]{titlesec}
% \titlespacing*
\titleformat{\section}{\normalfont\small\bfseries}{\thesection}{0em}{} % Remove space before and after section titles
\titleformat{\subsection}{\normalfont\small\bfseries}{\thesubsection}{0em}{} % Remove space before and after subsection titles
\titlespacing*{\section}{0pt}{0pt}{0pt} % Remove space before/after section titles
\titlespacing*{\subsection}{0pt}{0pt}{0pt} % Remove space before/after subsec titles

% -- Colors ----
\usepackage[dvipsnames]{xcolor}
\definecolor{dmm}{RGB}{192,192,192} % Define a custom dimmed text color
\definecolor{cmt}{RGB}{61,123,123}
\definecolor{err}{RGB}{128,0,0}
\definecolor{lbl}{RGB}{128,0,0}
% -- Math ------
\usepackage{amsmath}
\usepackage{mathtools}
\usepackage{amssymb}
\usepackage{cancel}
\usepackage{turnstile}%better vdash

% -- Lists -----
\usepackage[inline]{enumitem}
\setlist{noitemsep}% Remove vspace between items
% Set vspace before and after  list environments as well as the left margin
\setlist[itemize,1]{leftmargin=.6em,labelindent=0pt,labelsep=2pt,
  topsep=1pt,partopsep=1pt}
\setlist[enumerate,1]{leftmargin=1em,labelindent=0pt,labelsep=2pt,
  topsep=1pt,partopsep=1pt}
\setlist[itemize,2]{leftmargin=.3em,labelindent=1pt,topsep=1pt,partopsep=1pt}
\setlist[enumerate,2]{leftmargin=.8em,labelindent=1pt,topsep=1pt,partopsep=1pt}
\setlist[description]{labelwidth=\linewidth,font=\small\bfseries,leftmargin=1em,topsep=1pt,partopsep=1pt}
% -- Code listing ---
\usepackage{listings}
\lstset{
  aboveskip=4pt,
  belowskip=2pt,
  basicstyle=\small\ttfamily,
  breaklines=true,
  captionpos=b,
  commentstyle=\color{cmt},
  columns=flexible,
  frame=single,
  keepspaces=false,
  keywordstyle=\ttfamily\color{RoyalBlue},
  showspaces=false,
  showstringspaces=false,
  showtabs=false,
  tabsize=2,
}

% Parse Trees
\usepackage{tikz}
\usetikzlibrary{ arrows, arrows.meta, automata, bbox, calc, positioning, tikzmark, decorations.pathmorphing, decorations.pathreplacing, decorations.shapes, shapes.geometric, graphs}
% \usegdlibrary{trees}
\tikzset{
  >=stealth',
  node distance=1cm,
  recstate/.style={             % style without shape
    draw=blue!50,fill=blue!20,thick,font=\small\sffamily,rounded corners=3pt,
    minimum size=1cm,inner sep=1pt
  },
  % comp8712 mainly
  comp/.style={
    draw=blue!50,fill=blue!20,thick,font=\small\sffamily,rounded corners=3pt,
    minimum size=.5cm,inner sep=1pt
  },
  % LR DFA state
  lrst/.style={draw,shape=rectangle,minimum width=22mm,inner sep=2pt,
    font=\small,align=left},
  % Stack frame or AR
  ar/.style={draw,shape=rectangle,minimum width=36mm,font=\footnotesize\ttfamily,
    align=center},
  % label for stack
  arl/.style={font=\footnotesize\ttfamily,align=left},
  % notes for ar
  arn/.style={font=\footnotesize,align=left,text width=3cm},
  % liveness analysis
  la/.style={draw,shape=rectangle,minimum width=15mm,font=\footnotesize\ttfamily,
    align=center},
  every edge/.style={draw,auto,font=\small\sffamily},
  every loop/.style={looseness=4},
  initial text=start,
  % initial where=right
}

% algorithm
\usepackage{algpseudocode}
\usepackage{array}
\usepackage{annotate-equations}
% Place a figure env right here via [H] option
\usepackage{float}
\usepackage{tcolorbox}
% Side by side figure
\usepackage{subcaption}
% \usepackage{caption}
% \captionsetup{belowskip=0pt, aboveskip=0pt}
\usepackage{pifont}
\usepackage{fontawesome}
% -- Multi-Col layout --
\usepackage{multicol}

% No indentation
\setlength\parindent{0pt}
\setlength\abovedisplayskip{-5pt}
\setlength\belowdisplayskip{-5pt}
\setlength\abovedisplayshortskip{-4pt}
\setlength\belowdisplayshortskip{-4pt}
\setlength\tabcolsep{5pt}
\newcommand{\gor}{\;|\;}
\newcommand{\lrd}{\textcolor{Red}{\mathbf{.}}}
\newcommand{\mset}[1]{\{#1\}}
\newcommand{\num}{\texttt{\#}~}
\newcommand{\mfn}[1]{\mathsf{\uppercase{#1}}}
\newcommand{\mr}[1]{\textcolor{Magenta}{#1}}
\newcommand{\mb}[1]{\textcolor{Cerulean}{#1}}
\newcommand{\mo}[1]{\textcolor{Maroon}{#1}}
\newcommand{\ml}[1]{\textcolor{lbl}{#1}}
\newcommand{\dm}[1]{\textcolor{dmm}{#1}}
\newcommand{\hcancel}[2][red]{\setbox0=\hbox{$#2$}%
\rlap{\raisebox{.45\ht0}{\textcolor{#1}{\rule{\wd0}{1pt}}}}#2}
\renewcommand{\arraystretch}{1.2}
