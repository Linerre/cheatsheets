...
\begin{minipage}{0.45\linewidth}
  \begin{enumerate}[align=left]
    ...
    \item\label{nfa:erem} Check if any \(q_{i}\) in \textsc{eclose}(\(q\)) meet:
    \item[]\(\delta(q_{i},a) = s \rightarrow \delta(q,a) = s\) where \(s \in Q\)
    \item (optional) \textbf{copy} original \(\epsilon\)-NFA diagram
    \item[] \textbf{erase} all \(\epsilon\)-transitions and show connections found in step \ref{nfa:erem} in result DFA
...
\vspace{1em}\\
\begin{tikzpicture}[bezier bounding box,node distance=0.5cm]%reduce vertical space around graph
  \node[state, initial, minimum size=0.5cm] (q0) {$q_{0}$};
  \node[state, minimum size=0.5cm, right=of q0] (q1) {$q_{1}$};
  \node[state, accepting, minimum size=0.5cm, below=of q1] (q3) {$q_{3}$};
  \node[state, minimum size=0.5cm, right=of q3] (q2) {$q_{2}$};
  \node[state, accepting, minimum size=0.5cm, right=of q1] (q4) {$q_{4}$};
